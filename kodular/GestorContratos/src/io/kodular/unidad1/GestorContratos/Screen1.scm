#|
$JSON
{
  "authURL": ["creator.kodular.io"],
  "YaVersion": "242",
  "Source": "Form",
  "Properties": {
    "$Name": "Screen1",
    "$Type": "Form",
    "$Version": "44",
    "AboutScreen": "Gestor de contratos con persistencia local.",
    "AlignHorizontal": "3",
    "AppName": "Gestor de Contratos",
    "BackgroundColor": "&HFFF7FAFC",
    "PrimaryColor": "&HFF1A365D",
    "PrimaryColorDark": "&HFF153E75",
    "AccentColor": "&HFF2B6CB0",
    "ScreenOrientation": "portrait",
    "Scrollable": "True",
    "ShowListsAsJson": "True",
    "Sizing": "Responsive",
    "Theme": "AppTheme.Light.DarkActionBar",
    "Title": "Iniciar sesion",
    "TitleBarColor": "&HFF1A365D",
    "Uuid": "0",
    "$Components": [
      {
        "$Name": "Espacio1",
        "$Type": "SpaceView",
        "$Version": "1",
        "Height": "16",
        "Uuid": "1101"
      },
      {
        "$Name": "LabelTitulo",
        "$Type": "Label",
        "$Version": "10",
        "FontBold": "True",
        "FontSize": "22",
        "Text": "GESTOR DE CONTRATOS",
        "TextAlignment": "1",
        "TextColor": "&HFF1A365D",
        "Width": "-1090",
        "Uuid": "1102"
      },
      {
        "$Name": "LabelSub",
        "$Type": "Label",
        "$Version": "10",
        "FontSize": "14",
        "Text": "Inicie sesion para continuar",
        "TextAlignment": "1",
        "TextColor": "&HFF4A5568",
        "Width": "-1090",
        "Uuid": "1103"
      },
      {
        "$Name": "CampoUsuario",
        "$Type": "TextBox",
        "$Version": "13",
        "FontSize": "18",
        "Hint": "Usuario",
        "Width": "-1090",
        "Uuid": "1104"
      },
      {
        "$Name": "CampoClave",
        "$Type": "PasswordTextBox",
        "$Version": "6",
        "FontSize": "18",
        "Hint": "Clave",
        "Width": "-1090",
        "Uuid": "1105"
      },
      {
        "$Name": "Espacio2",
        "$Type": "SpaceView",
        "$Version": "1",
        "Height": "12",
        "Uuid": "1106"
      },
      {
        "$Name": "BotonEntrar",
        "$Type": "Button",
        "$Version": "13",
        "BackgroundColor": "&HFF2B6CB0",
        "FontBold": "True",
        "FontSize": "16",
        "Shape": "1",
        "Text": "Entrar",
        "TextColor": "&HFFFFFFFF",
        "Width": "-1090",
        "Uuid": "1107"
      },
      {
        "$Name": "BotonIrRegistro",
        "$Type": "Button",
        "$Version": "13",
        "BackgroundColor": "&HFF718096",
        "FontBold": "True",
        "FontSize": "16",
        "Shape": "1",
        "Text": "Crear usuario",
        "TextColor": "&HFFFFFFFF",
        "Width": "-1090",
        "Uuid": "1108"
      },
      {
        "$Name": "BotonMostrarRecuperar",
        "$Type": "Button",
        "$Version": "13",
        "BackgroundColor": "&HFFEDF2F7",
        "FontSize": "14",
        "Shape": "1",
        "Text": "Recordar clave por email",
        "TextColor": "&HFF1A365D",
        "Width": "-1090",
        "Uuid": "1109"
      },
      {
        "$Name": "CajaRecuperar",
        "$Type": "VerticalArrangement",
        "$Version": "10",
        "AlignHorizontal": "3",
        "Visible": "False",
        "Width": "-1090",
        "Uuid": "1110",
        "$Components": [
          {
            "$Name": "CampoEmailRecuperar",
            "$Type": "TextBox",
            "$Version": "13",
            "FontSize": "18",
            "Hint": "Email registrado",
            "Width": "-2",
            "Uuid": "1111"
          },
          {
            "$Name": "BotonEnviarClave",
            "$Type": "Button",
            "$Version": "13",
            "BackgroundColor": "&HFF2B6CB0",
            "FontBold": "True",
            "Text": "Enviar clave al correo",
            "TextColor": "&HFFFFFFFF",
            "Width": "-2",
            "Uuid": "1112"
          }
        ]
      },
      {
        "$Name": "Notificador",
        "$Type": "Notifier",
        "$Version": "11",
        "Uuid": "1113"
      },
      {
        "$Name": "TinyDB1",
        "$Type": "TinyDB",
        "$Version": "2",
        "Uuid": "1114"
      },
      {
        "$Name": "Correo1",
        "$Type": "ActivityStarter",
        "$Version": "6",
        "Action": "android.intent.action.SENDTO",
        "Uuid": "1115"
      }
    ]
  }
}
|#
